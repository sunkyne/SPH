...
class compare_dimension {
 public:
  ...
  bool operator()(const KdNode& p, const KdNode& q) {
    return (p.point->x()(d) < q.point->x()(d));
  }
  size_t d;
};
...
class DistanceMeasure {
 public:
  ...
  virtual double distance(CoordPoint p, CoordPoint q) = 0;
...
class DistanceL0 : virtual public DistanceMeasure {
 ...
 public:
  ...
  double distance(CoordPoint p, CoordPoint q) {
    size_t i;
    double dist, test;
    if (w) {
      dist = (*w)[0] * fabs(p->x()(0) - q->x()(0));
      for (i = 1; i < 3; i++) {
        test = (*w)[i] * fabs(p->x()(i) - q->x()(i));
        if (test > dist) dist = test;
      }
    } else {
      dist = fabs(p->x()(0) - q->x()(0));
      for (i = 1; i < 3; i++) {
        test = fabs(p->x()(i) - q->x()(i));
        if (test > dist) dist = test;
      }
    }
    return dist;
...
class DistanceL1 : virtual public DistanceMeasure {
 ...
 public:
  ...
  double distance(CoordPoint p, CoordPoint q) {
    ...
    double dist = 0.0;
    if (w) {
      for (i = 0; i < 3; i++) dist += (*w)[i] * fabs(p->x()(i) - q->x()(i));
    } else {
      for (i = 0; i < 3; i++) dist += fabs(p->x()(i) - q->x()(i));
    }
    return dist;
  }
  double coordinate_distance(double x, double y, size_t dim) {
...
class DistanceL2 : virtual public DistanceMeasure {
 ...
 public:
  ...
  double distance(CoordPoint p, CoordPoint q) {
    ...
    if (w) {
      for (i = 0; i < 3; i++)
        dist += (*w)[i] * (p->x()(i) - q->x()(i)) * (p->x()(i) - q->x()(i));
    } else {
      for (i = 0; i < 3; i++) dist += (p->x()(i) - q->x()(i)) * (p->x()(i) - q->x()(i));
    }
    return dist;
  }
  double coordinate_distance(double x, double y, size_t dim) {
...
KdTree::KdTree(const KdNodeVector* nodes, int distance_type /*=2*/) {
  ...
  Eigen::Vector3f temp;
  temp << nodes->begin()->point->x()(0), nodes->begin()->point->x()(1), nodes->begin()->point->x()(2);
  lobound = temp;
  upbound = temp;
  for (i = 1; i < nodes->size(); i++) {
    for (j = 0; j < dimension; j++) {
      val = allnodes[i].point->x()(j);
...
void KdTree::set_distance(int distance_type,
  ...
  if (distance_type == 0) {
    distance = std::make_shared< DistanceL0 >(weights);
  } else if (distance_type == 1) {
    distance = std::make_shared< DistanceL2 >(weights);
  } else {
    distance = std::make_shared< DistanceL2 >(weights);
  }
}
...
//--------------------------------------------------------------
std::shared_ptr< kdtree_node > KdTree::build_tree(size_t depth, size_t a, size_t b) {
  size_t m;
  double temp, cutval;
  auto node = std::make_shared< kdtree_node >();
  ...
  if (b - a <= 1) {
    ...
    cutval = allnodes[m].point->x()(node->cutdim);
    node->dataindex = m;
    if (m - a > 0) {
...
//--------------------------------------------------------------
void KdTree::k_nearest_neighbors(CoordPoint point, size_t k,
                                 KdNodeVector* result,
...
// distance from *point*.
//--------------------------------------------------------------
void KdTree::range_nearest_neighbors(CoordPoint point, double r,
                                     KdNodeVector* result) {
  KdNode temp;
  result->clear();
  if (3 != dimension)
...
//--------------------------------------------------------------
bool KdTree::neighbor_search(CoordPoint point, std::shared_ptr< kdtree_node > node,
  ...
  if (point->x()(node->cutdim) < node->point->x()(node->cutdim)) {
    if (node->loson)
...
//--------------------------------------------------------------
void KdTree::range_search(CoordPoint point, std::shared_ptr< kdtree_node > node,
...
bool KdTree::bounds_overlap_ball(CoordPoint point, double dist,
    std::shared_ptr< kdtree_node > node) {
  double distsum = 0.0;
  size_t i;
  for (i = 0; i < dimension; i++) {
    if (point->x()(i) < node->lobound(i)) {  // lower than low boundary
      distsum += distance->coordinate_distance(point->x()(i), node->lobound(i), i);
      if (distsum > dist) return false;
    } else if (point->x()(i) > node->lobound(i)) {  // higher than high boundary
      distsum += distance->coordinate_distance(point->x()(i), node->lobound(i), i);
      if (distsum > dist) return false;
    }
  }
...
// ball with radius *dist* around *point*
bool KdTree::ball_within_bounds(CoordPoint point, double dist,
    std::shared_ptr< kdtree_node > node) {
  size_t i;
  for (i = 0; i < dimension; i++)
    if (distance->coordinate_distance(point->x()(i), node->lobound(i), i) <= dist ||
        distance->coordinate_distance(point->x()(i), node->lobound(i), i) <= dist)
      return false;
  return true;
}
